#!/usr/bin/env python3
"""
Find markdown files that contain at least one of the specified tags.
"""
import os
import re
import sys
...
def find_files_with_tags(folder_path: str, target_tags: List[str]) -> List[Path]:
    """Find all markdown files that contain at least one of the target tags."""
    ...
    print(f"Searching {len(markdown_files)} markdown files for tags: {normalized_tags}")
    
    # Check each file for matching tags
    for md_file in markdown_files:
        file_tags = extract_tags_from_file(md_file)
        
        # Check if any target tag is in the file's tags
        if file_tags & normalized_tags:  # Set intersection
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python find_files_by_tags_OR.py <folder_path> <tag1> [tag2] [tag3] ...")
        print("Example: python find_files_by_tags_OR.py ./backend/tests/animal_example_clustered adult_crow adult_parrot")
    ...
    if matching_files:
        print(f"\nFound {len(matching_files)} files with at least one of the specified tags:")
        print("-" * 80)
