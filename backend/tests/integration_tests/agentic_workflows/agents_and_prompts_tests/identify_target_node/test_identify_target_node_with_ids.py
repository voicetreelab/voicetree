...
class TestIdentifyTargetNodeWithIDs:
    ...
    @pytest.mark.asyncio
    async def test_in_batch_chaining_to_new_node_unambiguous(self, prompt_loader):
        """
        Tests the critical logic of creating a new node and then routing subsequent,
        related segments to that same new node within the same request.
        This version is designed to be unambiguous, forcing the creation of a new node
        because the proposed feature is clearly distinct from existing nodes.
        """
        # Existing nodes are specific enough that the new concept won't fit.
        existing_nodes = """
        [
            {"id": 200, "name": "Mobile App Feature Backlog", "summary": "A list of all potential features for the mobile application."},
            {"id": 201, "name": "User Profile Section", "summary": "Tasks related to building the user profile, settings, and account info pages."},
            {"id": 202, "name": "Initial User Sign-up Journey", "summary": "The flow for a user creating an account and logging in for the first time."},
            {"id": 203, "name": "Backend API for User Data", "summary": "Endpoints for creating, reading, and updating user information."},
            {"id": 204, "name": "UI Component Library", "summary": "A collection of reusable React components for the frontend."}
        ]
        """

        # History sets up the problem: user confusion post-signup.
        transcript_history = """
        The user feedback from the last release was clear. People are getting confused right after they sign up.
        They don't know what to do first. We need to improve the initial experience to increase activation rates.
        """

        # A single utterance that introduces a new, major feature to solve the problem.
        # A "Project Templating System" is not just part of the "Sign-up Journey".
        current_utterance = """
        I have a bigger idea than just tutorials. Let's create a 'Project Templating System'.
        This system will allow users to start a new project from a pre-defined template, like 'Marketing Campaign'.
        Each template should then pre-populate the project with a list of relevant starting tasks.
        """

        segments = """
        [
            {"text": "I have a bigger idea than just tutorials. Let's create a 'Project Templating System'.", "is_routable": true},
            {"text": "This system will allow users to start a new project from a pre-defined template, like 'Marketing Campaign'.", "is_routable": true},
            {"text": "Each template should then pre-populate the project with a list of relevant starting tasks.", "is_routable": true}
        ]
        """
        ...
        node_a = result.target_nodes[0]
        node_b = result.target_nodes[1]
