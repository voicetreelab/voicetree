...
from backend.types import FilterResult
...
def get_neighborhood(
    ...
    radius: int,
    load_node_func: Optional[Callable[[str], NodeDict]] = None
) -> NodeList:
    """
...
def calculate_distance(node: NodeDict, target_id: str) -> int:
    """
    ...
    if 'distance_from_target' in node:
        distance_val = node['distance_from_target']
        return int(distance_val) if distance_val is not None else 0
    ...
    return int(depth_val) if depth_val is not None else 0
