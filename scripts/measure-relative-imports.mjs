...
const REPORT_PATH = join(REPO_ROOT, 'scripts', 'measure-relative-imports.report.json')
...
const RESOLUTION_EXTENSIONS = ['', '.ts', '.tsx', '.d.ts', '.js', '.jsx', '.json']

const sourceRoots = await discoverSourceRoots()
const files = await listProductionSourceFiles(sourceRoots)
if (files.length === 0) {
  throw new Error('measure-relative-imports found 0 production source files; check source globs')
}

const packageRootCache = new Map()
const packageNameCache = new Map()
const records = collectRelativeImportRecords(files)
const summary = summarizeRecords(records, files.length)

writeFileSync(REPORT_PATH, `${JSON.stringify(summary, null, 2)}\n`)
printReport(summary)
if (ENFORCE && summary.totals.bannedRelativeImports > 0) {
  console.error(`Relative import depth gate failed: ${summary.totals.bannedRelativeImports} banned relative import(s) found`)
  process.exit(1)
}
...
async function listProductionSourceFiles(roots) {
  const nested = await Promise.all(roots.map(async root => {
    ...
    return listProductionSourcesUnder(absoluteRoot)
  }))
...
function collectRelativeImportRecords(sourceFiles) {
...
function importSite(kind, literal, sourceFile) {
...
function classifyRelativeImport(importerPath, importerPackageRoot, importerPackageName, importSite, sourceFile) {
  const resolvedPath = resolveRelativeSpecifier(importerPath, importSite.specifier)
  ...
  return {
    ...
    column: importSite.column,
    kind: importSite.kind,
    specifier: importSite.specifier,
    depth: relativeDepthBucket(importSite.specifier),
    ...
    statement: statementTextAt(sourceFile, importSite.line),
  }
}
...
function directoryForPackageLookup(path) {
  if (existsSync(path)) {
    try {
...
function relativeDepthBucket(specifier) {
...
function summarizePerPackage(records) {
...
function summarizeWorstOffenders(records) {
...
function printReport(summary) {
  console.log('Relative import measurement report')
  console.log('')
  console.log(`Source files scanned: ${summary.sourceFileCount}`)
  console.log(`Option B cost, all relative imports: ${summary.totals.relativeImports}`)
  console.log(`Option A cost, cross-package relative imports: ${summary.totals.crossPackageRelativeImports}`)
  console.log(`Same-package relative imports: ${summary.totals.samePackageRelativeImports}`)
  console.log(`Banned by depth policy (cross-package or ../../+): ${summary.totals.bannedRelativeImports}`)
  console.log('')
  console.log('Same-package relative imports by depth:')
  for (const [depth, count] of Object.entries(summary.samePackageByDepth)) {
    console.log(`  ${depth}: ${count}`)
  }
  console.log('')
  console.log('Per-package totals:')
  for (const pkg of summary.perPackageTotals) {
    console.log(`  ${pkg.package}: ${pkg.relativeImports} total, ${pkg.crossPackageRelativeImports} cross-package, ${pkg.samePackageRelativeImports} same-package`)
  }
  console.log('')
  console.log('Cross-package relative imports:')
  if (summary.crossPackageRelatives.length === 0) {
    console.log('  none')
  } else {
    for (const record of summary.crossPackageRelatives) {
      console.log(`  ${record.importer}:${record.line} ${record.specifier} -> ${record.resolvedPath}`)
    }
  }
  console.log('')
  console.log('Deep same-package relative imports:')
  const deepSamePackage = summary.bannedRelatives.filter(record => record.classification === 'relative-same-package')
  if (deepSamePackage.length === 0) {
    console.log('  none')
  } else {
    for (const record of deepSamePackage) {
      console.log(`  ${record.importer}:${record.line} ${record.specifier} -> ${record.resolvedPath}`)
    }
  }
  console.log('')
