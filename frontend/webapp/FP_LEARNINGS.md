...
const effect: AppEffect<Graph> = apply_graph_deltas_to_db(graph, action)
...
**Reader pattern:**
```typescript
function apply_graph_deltas_to_db(graph, action): AppEffect<Graph> {
  return (env: Env) => TE.tryCatch(
    async () => {
      // Effect logic here
...
- Pure: `apply_graph_deltas_to_db()` - builds effect description
