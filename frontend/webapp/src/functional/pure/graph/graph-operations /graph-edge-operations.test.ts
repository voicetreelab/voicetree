...
import {
  ...
  removeOutgoingEdges,
  setOutgoingEdges
} from '@/functional/pure/graph/graph-operations /graph-edge-operations.ts'
import type { GraphNode } from '@/functional/pure/graph/types.ts'
import * as O from 'fp-ts/lib/Option.js'

describe('graph-edge-operations', () => {
  const createTestNode = (id: string, edges: string[] = []): GraphNode => ({
  ...
  describe('addOutgoingEdge', () => {
    it('should add an edge to a node with no edges', () => {
      ...
      expect(result.outgoingEdges).toEqual(['node2'])
  ...
  describe('removeOutgoingEdge', () => {
    it('should remove an edge from a node', () => {
      ...
      expect(result.outgoingEdges).toEqual(['node3'])
    })
  ...
  describe('removeOutgoingEdges', () => {
    it('should remove multiple edges from a node', () => {
      const node = createTestNode('node1', ['node2', 'node3', 'node4'])
