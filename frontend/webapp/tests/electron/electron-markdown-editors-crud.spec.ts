...
test.describe('Markdown Editor CRUD Tests', () => {
  ...
  test('should save markdown files in subfolders via editor', async ({ appWindow }) => {
    ...
    // Wait for graph to load with the test node
    await expect.poll(async () => {
      ...
      message: `Waiting for node ${nodeId} to load`,
      timeout: 10000
    }).toBe(true);

    // Read original file content for restoration later
    const testFilePath = path.join(FIXTURE_VAULT_PATH, '10_Setting_up_Agent_in_Feedback_Loop.md');
    ...
    // Click on node to open editor
    await appWindow.evaluate((id) => {
      const cy = (window as ExtendedWindow).cytoscapeInstance;
      if (!cy) throw new Error('Cytoscape not initialized');

      const node = cy.getElementById(id);
      if (node.length === 0) throw new Error(`node ${id} not found`);
    ...
    await expect.poll(async () => {
      return appWindow.evaluate((id) => {
        const editorWindow = document.getElementById(`window-editor-${id}`);
    ...
    await appWindow.waitForSelector(`#window-editor-${nodeId} .cm-editor`, { timeout: 5000 });

    // Modify content in the editor
    const testContent = '---\nnode_id: 10\ntitle: Setting up Agent in Feedback Loop (10)\n---\n### TEST MODIFICATION - This content was changed by the e2e test.\n\nSee [[11_Identify_Relevant_Test_for_Tree_Action_Decider_Workflow.md]] for details.';

    await appWindow.evaluate(({ editorId, newContent }: { editorId: string; newContent: string }) => {
      const w = (window as ExtendedWindow);
      const editor = w.testHelpers?.getEditorInstance(`editor-${editorId}`);
      if (editor) {
        editor.setValue(newContent);
      } else {
        throw new Error(`Editor instance not found for editor-${editorId}`);
      }
    }, { editorId: nodeId, newContent: testContent });
    ...
    await appWindow.evaluate((id: string) => {
      const closeButton = document.querySelector(`#window-editor-${id} .cy-floating-window-close`) as HTMLButtonElement;
    ...
    await expect.poll(async () => {
      return appWindow.evaluate((id: string) => {
        const cy = (window as ExtendedWindow).cytoscapeInstance;
        if (!cy) return false;
        const shadowNode = cy.getElementById(`editor-${id}`);
    ...
    await appWindow.waitForSelector(`#window-editor-${nodeId} .cm-editor`, { timeout: 500 });

    // Verify the editor shows the saved content
    const editorContent = await appWindow.evaluate((id: string) => {
      const w = (window as ExtendedWindow);
      const editor = w.testHelpers?.getEditorInstance(`editor-${id}`);
  ...
  test('should sync external file changes to open editors (bidirectional sync)', async ({ appWindow }) => {
    ...
    // Read original file content for restoration
    const testFilePath = path.join(FIXTURE_VAULT_PATH, '11_Identify_Relevant_Test_for_Tree_Action_Decider_Workflow.md');
    const originalContent = await fs.readFile(testFilePath, 'utf-8');
    console.log('Original file content:', originalContent.substring(0, 50) + '...');

    // Open the editor for node
    await appWindow.evaluate((id: string) => {
      const cy = (window as ExtendedWindow).cytoscapeInstance;
      if (!cy) throw new Error('Cytoscape not initialized');
      const node = cy.getElementById(id);
    ...
    const initialEditorContent = await appWindow.evaluate((id: string) => {
      const w = (window as ExtendedWindow);
      const editor = w.testHelpers?.getEditorInstance(`editor-${id}`);
      return editor?.getValue() || null;
    }, nodeId);
    ...
    const updatedEditorContent = await appWindow.evaluate((id: string) => {
      const w = (window as ExtendedWindow);
      const editor = w.testHelpers?.getEditorInstance(`editor-${id}`);
      return editor?.getValue() || null;
    }, nodeId);
