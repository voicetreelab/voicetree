...
DO NOT EVER RUN ANY GIT COMMANDS, NEVER REMOVE ANY FILES.

4. If all tests are passing from `npm run test`, bump the patch (e.g. 1.0.1 -> 1.0.2) version in package.json, 
5. and then package our app by doing `cd ../../ && ./build_and_package_all.sh --publish` you must run this in the foreground, with a 40 minute bash timeout. 
