...
test.describe('Context Node Agent Terminal E2E', () => {
  ...
  test('should set terminal title to "<AGENT_NAME>: <stripped_context_node_title>"', async ({ appWindow }) => {
    test.setTimeout(30000);

    console.log('=== STEP 1: Load the test vault (example_small) ===');
    const watchResult = await appWindow.evaluate(async (vaultPath) => {
      const api = (window as ExtendedWindow).electronAPI;
      if (!api) throw new Error('electronAPI not available');
      return await api.main.startFileWatching(vaultPath);
    ...
    // Wait for initial scan and graph to load
    await appWindow.waitForTimeout(3000);

    console.log('=== STEP 2: Select the parent node in cytoscape ===');
    const parentNodeId = '5_Immediate_Test_Observation_No_Output.md';
    ...
    console.log(`✓ Node ${parentNodeId} selected`);

    console.log('=== STEP 3: Trigger Cmd+Enter to spawn terminal ===');
    ...
    console.log('=== STEP 4: Check terminal title in DOM ===');
    // Look for the floating window title text
    const titleInfo = await appWindow.evaluate(() => {
      const titleElements = document.querySelectorAll('.cy-floating-window-title-text');
      if (titleElements.length === 0) {
        return { found: false, title: '', count: 0 };
      }
      // Get the most recently added terminal title (last one)
      const lastTitle = titleElements[titleElements.length - 1];
      return {
        found: true,
        title: lastTitle.textContent ?? '',
        count: titleElements.length
      };
    });

    console.log(`✓ Found ${titleInfo.count} floating window(s), title: "${titleInfo.title}"`);

    expect(titleInfo.found).toBe(true);

    // Title should be in format "<AGENT_NAME>: <stripped_title>"
    // The context node title is "CONTEXT for: <parent_title>" and we strip "CONTEXT for:"
    // AGENT_NAME comes from settings (could be user-configured or defaults)

    // Verify title has format "<AGENT_NAME>: <something>" (agent name followed by colon)
    const hasAgentNameFormat = /^[A-Za-z]+:\s*.+/.test(titleInfo.title);
    expect(hasAgentNameFormat).toBe(true);
    console.log('✓ Title has format "<AGENT_NAME>: <title>"');

    // Extract the agent name from title for logging
    const agentNameMatch = titleInfo.title.match(/^([A-Za-z]+):/);
    const agentName = agentNameMatch ? agentNameMatch[1] : 'unknown';
    console.log(`✓ Agent name in title: "${agentName}"`);

    // Verify title does NOT contain "CONTEXT for:" (this is the key behavior we're testing)
    expect(titleInfo.title).not.toContain('CONTEXT for:');
    console.log('✓ Title does not contain "CONTEXT for:" prefix (stripped correctly)');

    // Verify title contains part of the parent node name (proves stripping worked)
    // The parent node is "5_Immediate_Test_Observation_No_Output.md"
    // Context node title would be "CONTEXT for: 'Speaker observes no output...'"
    // After stripping "CONTEXT for:", it should contain the actual content
    expect(titleInfo.title.toLowerCase()).toContain('speaker');
    console.log('✓ Title contains parent node content');

    console.log('');
    console.log('=== TEST SUMMARY ===');
    console.log(`✓ Terminal title: "${titleInfo.title}"`);
    console.log(`✓ Format verified: "<AGENT_NAME>: <stripped_context_node_title>"`);
    console.log(`✓ "CONTEXT for:" prefix successfully stripped`);
    console.log('');
    console.log('✅ TERMINAL TITLE FORMAT TEST PASSED');
  });
});
