...
test.describe('Cmd+N and Cmd+Enter Hotkeys (Browser)', () => {
  ...
  // Skip: Terminal spawning requires IPC to main process which isn't available in browser tests.
  // The full terminal spawning flow is tested in electron e2e tests (electron-terminal-auto-command.spec.ts).
  test.skip('should open terminal with Cmd+Enter when node is selected', async () => {
    // Terminal spawning tested in electron e2e tests
  });
