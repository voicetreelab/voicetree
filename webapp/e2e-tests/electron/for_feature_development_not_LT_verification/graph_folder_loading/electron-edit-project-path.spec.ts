...
import { test, type ExtendedWindow } from './electron-edit-project-path/fixtures';
import { assertOpenedForEditing, openFirstProjectPathForEditing } from './electron-edit-project-path/path-editing';

test.describe('Edit Path (Inline Rename) E2E', () => {
  test('Test Scenario 1: Edit Root Path to Subfolder', async ({ appWindow }) => {
    test.setTimeout(30000);

    // Test editing the default root path to point to write-project subfolder
    ...
    assertOpenedForEditing(await openFirstProjectPathForEditing(appWindow));

    // Wait for edit mode
    await appWindow.waitForTimeout(300);
    console.log('Edit mode activated');

    console.log('=== STEP 2: Change text to write-project ===');
    ...
    await editInput.fill('write-project');
    ...
    console.log('Final project paths:', finalPaths);

    // Should contain write-project in one of the paths
    const hasWriteProject = finalPaths.some((p: string) => p.includes('write-project'));
    expect(hasWriteProject).toBe(true);
  ...
  test('Test Scenario 2: Edit Write Path (root path is write path)', async ({ appWindow }) => {
    ...
    // We'll edit it to point to write-project subfolder
    ...
    console.log('=== STEP 2: Change to write-project ===');
    const editInput = appWindow.locator('.absolute.bottom-full input[type="text"]:not([placeholder])');
    await editInput.clear();
    await editInput.fill('write-project');
    ...
    console.log('Final write path:', finalWriteFolderPath);
    expect(finalWriteFolderPath).toContain('write-project');

    console.log('=== STEP 5: Assert nodes from write-project are loaded into graph ===');
    ...
    // write-project/node-a.md should be loaded into the graph
    const hasNodeA = graphNodes.some((id: string) => id.includes('write-project') && id.includes('node-a'));
  ...
  test('Test Scenario 4: Edit with Absolute Path', async ({ appWindow }) => {
    ...
    const absoluteTargetPath = await fs.mkdtemp(path.join(os.tmpdir(), 'voicetree-absolute-project-'));
