/**
 * BEHAVIORAL SPEC:
 * E2E test for "Load Folder with Project Config" functionality.
 *
 * This test verifies:
 * 1. Project config persists across app restarts
 ...
 *
 * Based on: voicetree-18-1/multiproject-spec-load-folder.md
 */
 ...
 *
 * Setup:
 *   - write-project/node-a.md (links to linked-node)
 *   - read-project/linked-node.md (linked by node-a)
 *   - read-project/unlinked-node.md (not linked)
 *
 * Expected:
 *   - writeFolderPath is restored to 'write-project'
 *   - readPaths includes 'read-project'
...
const testPersistence = base.extend<{
  ...
  writeProjectPath: async ({ testDir }, use) => {
    const writeProjectPath = path.join(testDir, 'write-project');
    await fs.mkdir(writeProjectPath, { recursive: true });

    // Create node-a that links to linked-node in read-project
    await fs.writeFile(
      path.join(writeProjectPath, 'node-a.md'),
      `# Node A

This is the main node in the write project.

Links to [[linked-node]] in the read project.
`
    );

    await use(writeProjectPath);
  },
  ...
  readProjectPath: async ({ testDir }, use) => {
    ...
    await fs.writeFile(
      path.join(readProjectPath, 'linked-node.md'),
      `# Linked Node
...
`
    ...
    await fs.writeFile(
      path.join(readProjectPath, 'unlinked-node.md'),
      `# Unlinked Node
...
testPersistence.describe('Project Config Persistence E2E', () => {
  testPersistence('should persist project config and use lazy loading on reload', async ({
    testDir,
    writeProjectPath,
    readProjectPath,
    tempUserDataPath
    ...
    console.log('=== PHASE 1: Initial launch - configure projectConfig ===');

    // Phase 1: Launch app and configure project config manually via API
    // Start with empty config (no projectConfig preset)
    ...
    window1.on('console', msg => {
      const text = msg.text();
      if (text.includes('[loadFolder]') || text.includes('projectConfig')) {
    ...
    expect(projectPathsBefore.length).toBeGreaterThanOrEqual(2);
    ...
    window2.on('console', msg => {
      const text = msg.text();
      if (text.includes('[loadFolder]') || text.includes('projectConfig') || text.includes('Lazy')) {
    ...
    expect(restoredWriteFolderPath).toBe(writeProjectPath);
    ...
    expect(restoredReadOnLinkPaths).toContain(readProjectPath);
    ...
    console.log('Project Config Persistence test completed:');
...
testDefaultConfig.describe('Default Project Config Creation E2E', () => {
  testDefaultConfig('should create default config with allowlist patterns when loading fresh directory', async ({
    ...
    console.log('=== Setup: Fresh directory with no projectConfig ===');
    console.log('Test directory:', testDir);

    // Create config with lastDirectory but NO projectConfig
    const configPath = path.join(tempUserDataPath, 'voicetree-config.json');
    await fs.writeFile(
      configPath,
      JSON.stringify({ lastDirectory: testDir }, null, 2),
    ...
    window.on('console', msg => {
      const text = msg.text();
      if (text.includes('[loadFolder]') || text.includes('projectConfig') || text.includes('allowlist')) {
    ...
    // Verify projectConfig was created for the testDir
    expect(savedConfig.projectConfig).toBeDefined();
    expect(savedConfig.projectConfig[testDir]).toBeDefined();
    expect(savedConfig.projectConfig[testDir].writeFolderPath).toBeDefined();
