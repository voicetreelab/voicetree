...
test.describe('Folder Compound Nodes (v2.1)', () => {
    ...
    test('deleting all files in a folder cleans up empty compound', async ({ appWindow, projectRoot }) => {
        test.setTimeout(60000);
        await waitForGraphLoaded(appWindow, 3);
        ...
        await fs.rm(path.join(projectRoot, 'utils', 'logger.md'));
        await fs.rm(path.join(projectRoot, 'utils', 'config.md'));
