...
export function promptFileMode(promptFile: string): number {
  return statSync(promptFile).mode & 0o777;
}
...
export async function resolveVaultWriteFolder(
  parentDir: string,
): Promise<string> {
...
export async function discoverMcpUrl(window: Page): Promise<string> {
...
export async function bootstrapCallerTerminal(
  window: Page,
  parentNodeId: string,
  callerId: string,
): Promise<void> {
...
export const test = base.extend<PhaseSixFixtures>({
...
export async function firstLoadedWindow(
