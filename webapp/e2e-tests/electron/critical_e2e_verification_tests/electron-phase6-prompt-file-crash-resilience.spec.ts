...
function reapStaleTestTmuxSessions(extraNames: ReadonlyArray<string> = []): void {
...
async function resolveVaultWritePath(parentDir: string): Promise<string> {
    ...
    const voicetreeDir = entries.find(e => e.isDirectory() && /^voicetree(-\d{1,2}-\d{1,2})?$/.test(e.name));
    if (!voicetreeDir) throw new Error(`No voicetree-* subfolder under ${parentDir}; file watching may not have initialised yet`);
...
async function writeSettings(userDataDir: string, vaultPath: string, agentPrompt: string): Promise<void> {
    ...
    await fs.writeFile(path.join(userDataDir, 'projects.json'), JSON.stringify([{
    ...
    await fs.writeFile(path.join(userDataDir, 'voicetree-config.json'), JSON.stringify({
        lastDirectory: vaultPath,
    }, null, 2), 'utf8');
}

async function launchElectronApp(userDataDir: string, vaultPath: string): Promise<ElectronApplication> {
    ...
    proc?.stdout?.on('data', (c: Buffer) => console.log(`[MAIN] ${c.toString().trimEnd()}`));
    proc?.stderr?.on('data', (c: Buffer) => console.error(`[MAIN ERR] ${c.toString().trimEnd()}`));
    return app;
}
...
async function discoverMcpUrl(window: Page): Promise<string> {
    const port = await window.evaluate(async () => {
        ...
        return await (api.main as unknown as { getMcpPort: () => Promise<number> }).getMcpPort();
    });
    return `http://127.0.0.1:${port}/mcp`;
}

async function bootstrapCallerTerminal(window: Page, parentNodeId: string, callerId: string): Promise<void> {
    const res = await window.evaluate(async ({ parentNodeId: nodeId, callerId: id }) => {
        const api = (window as unknown as ExtendedWindow).electronAPI;
    ...
    expect(res.success, `caller terminal spawn failed: ${JSON.stringify(res)}`).toBe(true);
}
...
const test = base.extend<PhaseSixFixtures>({
    ...
    phaseSixPrompt: async ({}, use) => {
        const promptHash = createHash('sha1').update(`phase6-${Date.now()}-${Math.random()}`).digest('hex').slice(0, 12);
