...
// Forest-of-Tidy-Trees engine. A VoiceTree vault is a forest of mostly
// trees/chains, so we lay out every connected component as its own size-aware
// d3 tidy tree (pure math in mindmapForest) and pack the trees into a compact
// rectangle. Deterministic, iteration-free, crossing-free along tree edges, and
// overlap-free by construction; the VPSC pass is only a safety net for any
// residual card overhang. No force backend re-disperses the packing, so there
// is no pack-before-backend dispersion to undo (the autoLayout linchpin) here.
const runMindmap = async ({
  ...
  if (graph.nodes.length === 0) return;
  const gap = Math.max(40, numericOption(config.cola.nodeSpacing, 120));
  const positions = computeForestLayout(
    graph.nodes.map((node) => ({ id: node.id, size: node.data.size })),
    graph.edges.map((edge) => ({ source: edge.source, target: edge.target })),
    gap,
  );
  const movableIds = movableNodeIds(graph.nodes, fixedNodeIds, movableNodes);
  applyLayoutEnginePositions(cy, positions, movableIds);
  finishOverlaps(cy, graph.nodes, movableIds, fixedNodeIds, gap / 4);
};
