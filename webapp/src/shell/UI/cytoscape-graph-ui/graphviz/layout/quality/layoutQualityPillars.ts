...
import type { LayoutQualityBox, LayoutQualityEdge, LayoutQualityNode, LayoutScoringConfig } from './layoutQualityTypes';
import {
  type Point,
  bandScore,
  boxArea,
...
export function edgeSegments(edges: readonly LayoutQualityEdge[], byId: Map<string, LayoutQualityNode>): EdgeSeg[] {
  const segs: EdgeSeg[] = [];
...
export function nodeOverlapPillar(boxes: readonly LayoutQualityBox[]): {
...
export function titleLegibilityPillar(
  nodes: readonly LayoutQualityNode[],
  boxes: readonly LayoutQualityBox[],
...
export function edgeLengthPillar(
  edges: readonly LayoutQualityEdge[],
  byId: Map<string, LayoutQualityNode>,
...
export function whitespacePillar(
  ...
  segs: readonly EdgeSeg[],
  config: LayoutScoringConfig,
): { score: number; inkArea: number; density: number; bbox: LayoutQualityBox; bboxArea: number } {
  ...
  const bbox: LayoutQualityBox = boxes.length > 0 ? { x1, y1, x2, y2 } : { x1: 0, y1: 0, x2: 0, y2: 0 };
...
function boundingBoxOf(ids: readonly string[], boxById: Map<string, LayoutQualityBox>): LayoutQualityBox {
  let x1 = Infinity, y1 = Infinity, x2 = -Infinity, y2 = -Infinity;
  for (const id of ids) {
...
export function componentSeparationPillar(
  ...
  boxById: Map<string, LayoutQualityBox>,
  medianExtent: number,
  config: LayoutScoringConfig,
...
export function componentsOf(nodes: readonly LayoutQualityNode[], edges: readonly LayoutQualityEdge[]): Map<string, string[]> {
