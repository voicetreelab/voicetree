...
// `LayoutQualityBox` rather than `Rect` to stay distinct from graph-model's spatial
// `Rect` ({minX,maxX,...}) — different shape, different domain.
export type LayoutQualityBox = {
  readonly x1: number;
  readonly y1: number;
  readonly x2: number;
  readonly y2: number;
...
export type LayoutQualityNode = {
  ...
  readonly titleBox?: LayoutQualityBox;
