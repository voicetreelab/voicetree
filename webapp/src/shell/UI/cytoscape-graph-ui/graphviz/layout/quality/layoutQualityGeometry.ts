...
import type { LayoutQualityBox, LayoutQualityNode } from './layoutQualityTypes';
...
export function nodeBox(node: LayoutQualityNode): LayoutQualityBox {
  const hw = node.width / 2;
  const hh = node.height / 2;
  return { x1: node.x - hw, y1: node.y - hh, x2: node.x + hw, y2: node.y + hh };
}

export function boxArea(b: LayoutQualityBox): number {
  return Math.max(0, b.x2 - b.x1) * Math.max(0, b.y2 - b.y1);
}

export function boxOverlapArea(a: LayoutQualityBox, b: LayoutQualityBox): number {
...
export function boxGap(a: LayoutQualityBox, b: LayoutQualityBox): number {
...
export function effRadius(node: LayoutQualityNode): number {
...
export function segmentCrossesBox(p1: Point, p2: Point, b: LayoutQualityBox): boolean {
