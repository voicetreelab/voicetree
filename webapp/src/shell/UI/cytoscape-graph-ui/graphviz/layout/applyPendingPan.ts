...
export function applyPendingPan(cy: Core, action: PanAction): void {
  switch (action.kind) {
    case 'fit-non-folder-elements': {
      ...
      const eles: CollectionReturnValue = cy.elements().filter(ele => !ele.data('isFolderNode')) as CollectionReturnValue;
    ...
    case 'fit-non-folder-nodes': {
      ...
      const nodes: CollectionReturnValue = cy.nodes().filter(n => !n.data('isFolderNode')) as CollectionReturnValue;
      cyFitCollectionByAverageNodeSize(cy, nodes, action.targetFraction);
      break;
    }
    case 'smart-center-with-neighbors': {
      const target: CollectionReturnValue = cy.getElementById(action.nodeId);
      if (target.length > 0) {
        const neighborhood: CollectionReturnValue = target.closedNeighborhood().nodes().filter(n => !n.data('isFolderNode')) as CollectionReturnValue;
        cySmartCenter(cy, neighborhood);
      }
      break;
    }
    case 'smart-center': {
      const node: CollectionReturnValue = cy.getElementById(action.nodeId);
