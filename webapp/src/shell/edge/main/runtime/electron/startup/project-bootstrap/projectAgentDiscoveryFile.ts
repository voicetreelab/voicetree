/**
 * Project-open advertisement of the `vt` CLI for user-launched coding agents.
 *
 ...
 * OpenCode, …) directly in their project never sees the manual
 ...
 *   - If `<project>/CLAUDE.md` exists, append (or replace) a fenced
 *     VoiceTree section advertising the `vt` CLI. Idempotent: the
 *     section is delimited by sentinels so subsequent project opens
 *     overwrite the existing block rather than nesting.
 *   - Otherwise, write `<project>/.voicetree/AGENTS.md` containing the
 ...
 * CLAUDE.md / AGENTS.md the next time the project is opened.
 */
...
/**
 * Write or update the VoiceTree CLI discovery file inside `projectDir`.
 *
 * Strategy:
 *   - If `<projectDir>/CLAUDE.md` exists, append / replace the VoiceTree
 *     section there. Other content in the file is left intact.
 *   - Otherwise, write `<projectDir>/.voicetree/AGENTS.md` with the
...
export async function writeProjectAgentDiscoveryFile(
    ...
    const dotVoicetreeDir: string = getProjectDotVoicetreePath(projectDir)
