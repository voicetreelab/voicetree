...
function setupStandardMocks(graphOverride?: Graph): void {
    ...
    vi.mocked(postDeltaThroughDaemonWithEditors).mockResolvedValue(undefined)
}
...
describe('MCP create_graph tool', () => {
    ...
    describe('line length blocking', () => {
        ...
        it('blocks all nodes if any single node is too long', async () => {
            ...
            expect(postDeltaThroughDaemonWithEditors).not.toHaveBeenCalled()
        })
    })
    ...
    describe('single node creation', () => {
        ...
        it('uses agent color and name from terminal record', async () => {
            ...
            const deltaCalls: GraphDelta[] = vi.mocked(postDeltaThroughDaemonWithEditors).mock.calls.map(
    ...
    describe('multi-node tree creation', () => {
        ...
        it('creates parents before children (topological order)', async () => {
            ...
            const firstDelta: GraphDelta = vi.mocked(postDeltaThroughDaemonWithEditors).mock.calls[0][0]
        ...
        it('preserves labeled parent edges for locally created nodes', async () => {
            ...
            const calls: Array<[GraphDelta, boolean | undefined]> =
                vi.mocked(postDeltaThroughDaemonWithEditors).mock.calls as Array<[GraphDelta, boolean | undefined]>
        ...
        it('positions children spread vertically from parent', async () => {
            ...
            const calls: Array<[GraphDelta, boolean | undefined]> = vi.mocked(postDeltaThroughDaemonWithEditors).mock.calls as Array<[GraphDelta, boolean | undefined]>
            const creationDelta: GraphDelta = calls[0][0]
