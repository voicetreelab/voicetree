...
// cheapest possible cost. Spawns the real `webapp/bin/vt` script as a child
// process — no Electron, no playwright, no MCP daemon roundtrip. Three
// scenarios that exercise the layers most likely to break:
//
//   1. `vt --help`: proves the wrapper loads tsx, resolves @/* aliases via the
//      pinned tsconfig, and the dispatcher renders help without crashing.
...
const WEBAPP_DIR: string = resolve(TEST_FILE_DIR, '../../../../..')
const VT_BIN: string = join(WEBAPP_DIR, 'bin', 'vt')
...
describe('vt CLI smoke (Tier 1)', () => {
    ...
    it('loads and prints help from outside webapp/', async () => {
        ...
        // dir. The vt wrapper pins TSX_TSCONFIG_PATH so @/* aliases resolve.
