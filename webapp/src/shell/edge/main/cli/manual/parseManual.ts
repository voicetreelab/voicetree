/**
 * Pure parser for `tools/prompts/cli-manual.md`. Splits the file into
 * `ManualTool` entries that the parity lint and `vt manual` command can both
 * consume. Format (every rule is enforced by the parity lint — drift will fail
 * the test rather than silently mis-parse):
 *
