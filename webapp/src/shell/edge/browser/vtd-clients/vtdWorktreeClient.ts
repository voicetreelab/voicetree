...
// (from the protocol's GATEWAY_METHODS.worktree) and project each response onto the
...
export function vtdListWorktrees(vtdUrl: string, token: string): Promise<readonly WorktreeInfo[]> {
    return callVtdRpc(vtdUrl, token, GATEWAY_METHODS.worktree.list, {})
}
...
export async function vtdCreateWorktree(vtdUrl: string, token: string, worktreeName: string): Promise<string> {
    const res = await callVtdRpc<WorktreeCreate.Response>(vtdUrl, token, GATEWAY_METHODS.worktree.create, {worktreeName})
    return res.path
}
...
export async function vtdGenerateWorktreeName(vtdUrl: string, token: string, nodeTitle: string): Promise<string> {
    const res = await callVtdRpc<WorktreeGenerateName.Response>(vtdUrl, token, GATEWAY_METHODS.worktree.generateName, {nodeTitle})
    return res.name
}
...
export function vtdRemoveWorktree(
    ...
    return callVtdRpc(vtdUrl, token, GATEWAY_METHODS.worktree.remove, {worktreePath, force})
}
...
export async function vtdRemoveWorktreeCommand(
    ...
    const res = await callVtdRpc<WorktreeRemoveCommand.Response>(vtdUrl, token, GATEWAY_METHODS.worktree.removeCommand, {worktreePath, force})
    return res.command
}
