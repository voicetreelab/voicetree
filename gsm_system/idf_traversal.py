...
def find_child_references(parent_filename: str, markdown_dir: Path, file_cache: Dict[str, str]) -> List[str]:
    """Find all files that reference the parent file (any markdown link to parent makes it a child)."""
    children = []
    ...
    for md_file in parent_dir.glob('*.md'):
        ...
        # Match patterns like: [[filename.md]] or [[filename]] anywhere in the content
