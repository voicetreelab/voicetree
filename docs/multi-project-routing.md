...
How public VT routes `create_graph` writes across loaded project paths, and how an external workflow author composes those primitives to target a chosen project.
...
2. **Absolute `outputPath`** → resolved as-is, then checked against the loaded project paths.
3. **Relative `outputPath`** → resolved against the **`writePath`** (not against the agent's CWD), then checked against the loaded project paths.
4. **Resolved path inside any loaded project path** → accepted.
5. **Resolved path outside every loaded project path** → rejected with: `outputPath "<x>" resolves to "<y>" which is outside the loaded project paths. Choose a path inside one of: <list>`.
...
For an external workflow that wants to route writes into a specific loaded project:
...
A workflow that wants per-agent project routing currently composes by:
...
- Relative `outputPath` is resolved against `writePath`, not against the agent's CWD. Agents must pass absolute paths to target other loaded projects.
...
1. The current `outputPath` mechanism already permits writing into any loaded project path; secondary projects are not read-only at the `create_graph` layer. The gap is about default routing ergonomics, not about reachability.
