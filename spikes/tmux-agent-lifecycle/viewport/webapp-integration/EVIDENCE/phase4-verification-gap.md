...
- ESLint. `npm --workspace webapp run lint:run -- --quiet` failed because `eslint` is not installed in this checkout; `npx eslint ...` failed because `/Users/bobbobby/.npm/_cacache` contains root-owned files and cannot be written by this process.
