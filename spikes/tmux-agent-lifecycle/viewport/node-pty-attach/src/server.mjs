...
function attachTmux(ws, req) {
  const url = new URL(req.url, `http://${req.headers.host}`);
  const name = decodeURIComponent(url.pathname.replace(/^\/attach\//, ''));
...
server.on('upgrade', (req, socket, head) => {
  ...
  wss.handleUpgrade(req, socket, head, (ws) => attachTmux(ws, req));
});

server.listen(port, () => {
  console.log(`BF-207 node-pty attach bridge listening on http://127.0.0.1:${port}`);
});
