...
def test_complex_tree_creation():
    ...
    try:
        ...
        if result1 is None:
            raise RuntimeError("Pipeline returned None for transcript 1 - this should not happen with proper error handling")
        
        print_detailed_results(result1)
        results.append(result1)
        
        # Update existing nodes based on result1 - safe access with None check
        ...
        # Check for None result
        if result2 is None:
            raise RuntimeError("Pipeline returned None for transcript 2 - this should not happen with proper error handling")
        
        print_detailed_results(result2)
        results.append(result2)
        
        # Update existing nodes based on result2 - safe access with None check
        ...
        # Check for None result
        if result3 is None:
            raise RuntimeError("Pipeline returned None for transcript 3 - this should not happen with proper error handling")
        ...
        # Count total nodes created - safe access with None checks
        total_new_nodes = sum(len(r.get("new_nodes", [])) for r in results if r is not None)
        ...
        create_count = 0
        for r in results:
            if r is not None and r.get("integration_decisions"):
...
def test_single_transcript():
    ...
    try:
        ...
        # Check for None result
        if result is None:
            raise RuntimeError("Pipeline returned None - this should not happen with proper error handling")
