...
- [x] 1.4 Update `webapp/src/shell/edge/main/runtime/electron/app/main.ts:262-267` to either drop the startup reconciliation call (now redundant with `onVaultOpened`) or resolve `projectRoot` through the graph bridge. Delete the `process.env.VOICETREE_VAULT_PATH`-driven branch.
