...
// Generated data/analysis output files whose content IS the historical vocabulary
// being measured — the banned terms are the data, not production code. These are
// excluded from all vocabulary scanning for the same reason as lockfiles: they are
// machine-written artifacts committed as reference data, not authored source.
const EXCLUDED_DATA_PATHS: readonly string[] = [
    // Frequency table of historical agent search keywords mined from .claude sessions.
    // The keys ARE the search terms (including legacy names like "vault", "mcp", etc.);
    // editing them would falsify the dataset. Generated by extract_search_keywords.py.
    'scripts/data_analysis/context_graph_optimization/keyword_counts.json',
]
...
function isExcludedDataPath(repoRelativePath: string): boolean {
    return EXCLUDED_DATA_PATHS.some(p => repoRelativePath === p)
}
...
// A git-tracked path is excluded when:
//   - any directory segment is an excluded dir (by exact name or prefix), OR
//   - its filename is an excluded file (dependency lockfiles), OR
//   - it is a specific data/analysis output file (EXCLUDED_DATA_PATHS).
// Build/cache dirs are untracked by construction, so the tracked enumeration in
// checkProjectVocabulary needs no other path filter.
function isExcludedTrackedPath(repoRelativePath: string): boolean {
    const segments = repoRelativePath.split('/')
    const fileName = segments[segments.length - 1]
    const dirSegments = segments.slice(0, -1)
    return dirSegments.some(isExcludedDir) || isExcludedFile(fileName) || isExcludedDataPath(repoRelativePath)
}
