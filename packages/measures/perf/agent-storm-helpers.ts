...
import type { Action, FakeAgentScript } from '../../../tools/vt-fake-agent/src/types.ts'
...
export function buildFakeAgentScript(nodesPerAgent: number): FakeAgentScript {
    const actions: Action[] = []
    for (let i = 0; i < nodesPerAgent; i++) {
        // One `create_nodes` action per node so each lands as its own graph
        // delta — that per-delta cadence is exactly what stresses graphd's
        // per-delta whole-graph projection. Must match the fake-agent
        // executor's Action contract (`create_nodes` with a `nodes[]`); the
        // older singular `create_node` shape was silently rejected as an
        // "Unknown action type", so the harness measured zero agent load.
        actions.push({
            type: 'create_nodes',
            nodes: [{
                title: `Perf Node ${i}`,
