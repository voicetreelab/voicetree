...
import type { NodeDelta, GraphDelta, GraphNode, NodeUIMetadata } from '@vt/graph-model/graph'
import {
    markRecentDelta,
    isOurRecentDelta,
    clearRecentDeltas,
...
describe('recent-deltas-store', () => {
    ...
    describe('markRecentDelta + isOurRecentDelta for upserts', () => {
        ...
        it('should return false for slightly different content (exact match required)', () => {
            const delta: NodeDelta = makeUpsertDelta('test-node', 'hello world this is a test')
            markRecentDelta(delta)

            // Content with small differences (typos, minor edits) - should NOT match
            // because exact normalized comparison prevents suppressing external edits
