...
export interface ProjectGraphDerivedFolderTreeArgs {
  ...
  readonly writePath: AbsolutePath | null
}
...
export function projectGraphDerivedFolderTree(
  ...
  const { graph, projectRoot, readPaths, vaultPaths, writePath } = args
  ...
  return buildFolderTree(directoryEntry, loadedPaths, writePath, graphFilePathsSet)
}
