...
export type ProjectMutationSuccess =
...
export function decodeProjectPath(encodedPath: string):
...
export function composeProjectState(input: {
...
export function classifyRemoveReadPathResult(
  result: ProjectMutationResult,
): { readonly kind: 'success' } | ProjectMutationError {
  if (result.success) return { kind: 'success' }
