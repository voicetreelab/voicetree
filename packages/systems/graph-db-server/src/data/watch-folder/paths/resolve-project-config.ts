 ...
 * - resolveAllowlistForProject: Resolve the full project path configuration for a project,
...
import { getExpandedFolderPathsForProject } from "../folder-visibility-active-view";
import {
...
/**
 * Resolve the project path configuration for a project.
 *
 * If saved project config exists, it is authoritative - use it directly.
...
export async function resolveAllowlistForProject(
    ...
    const savedProjectConfig: ProjectConfig | undefined = await getProjectConfigForDirectory(watchedDir);
    ...
    if (!savedProjectConfig?.writeFolderPath) {
        return null;
    }

    // Resolve writeFolderPath to absolute
    const absoluteWriteFolderPath: string = resolveWriteFolderPath(watchedDir, savedProjectConfig.writeFolderPath);
    ...
    for (const expandedPath of await getExpandedFolderPathsForProject(watchedDir)) {
