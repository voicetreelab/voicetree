...
async function createTempVault(): Promise<string> {
  return await mkdtemp(join(tmpdir(), 'graphd-vault-views-test-'))
}

describe('vaultViews routes', () => {
  let vault: string
  ...
  test('creates, lists, activates, clones, and deletes views', async () => {
    ...
    const initial = await (await fetch(`${baseUrl}/vault/views`)).json()
    expect(initial).toMatchObject([{ name: 'main', isActive: true }])

    const created = await fetch(`${baseUrl}/vault/views`, {
      method: 'POST',
    ...
    const activated = await fetch(`${baseUrl}/vault/views/${alt.viewId}/activate`, {
      method: 'POST',
    })
    expect(activated.status).toBe(200)
    expect(await activated.json()).toMatchObject({ viewId: alt.viewId, isActive: true })

    const deleteActive = await fetch(`${baseUrl}/vault/views/${alt.viewId}`, {
      method: 'DELETE',
    })
    expect(deleteActive.status).toBe(409)
    expect(await deleteActive.json()).toMatchObject({ error: 'active view' })

    const cloned = await fetch(`${baseUrl}/vault/views/${alt.viewId}/clone`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
    ...
    const deleted = await fetch(`${baseUrl}/vault/views/${copy.viewId}`, {
      method: 'DELETE',
    })
