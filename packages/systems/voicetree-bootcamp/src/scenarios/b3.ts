 ...
 * resolves to some file in the project.
 */
...
const TASK_PROMPT = `This project has five flat notes (note-1.md … note-5.md) with wikilinks between
...
After reorganizing, verify that every wikilink in the project still resolves to
...
export const b3: ScenarioSpec = {
    ...
    async setup(projectDir) {
        await writeFile(path.join(projectDir, 'note-1.md'), '# Note 1\n\nSee [[note-2]] and [[note-3]].\n')
        await writeFile(
            path.join(projectDir, 'note-2.md'),
            '# Note 2\n\nReferenced by [[note-1]]. See also [[note-4]].\n',
        )
        await writeFile(path.join(projectDir, 'note-3.md'), '# Note 3\n\nStandalone chapter content.\n')
        await writeFile(path.join(projectDir, 'note-4.md'), '# Note 4\n\nLinked from [[note-2]].\n')
        await writeFile(path.join(projectDir, 'note-5.md'), '# Note 5\n\nUnlinked draft.\n')
    },
    ...
    async successCriteria(projectDir): Promise<SuccessResult> {
        for (const rel of EXPECTED_PATHS) {
            const present = await fileExists(path.join(projectDir, rel))
            if (!present) {
                return {passed: false, detail: `expected path missing: ${rel}`}
            }
        }
        if (await fileExists(path.join(projectDir, 'note-1.md'))) {
            return {
                passed: false,
        ...
        const mdFiles = await listMarkdownFiles(projectDir)
        ...
        for (const file of mdFiles) {
            ...
            for (const link of parseWikilinks(raw)) {
                ...
                if (!basenamesLower.has(target.toLowerCase())) {
                    return {
                        passed: false,
                        detail: `dangling wikilink [[${link}]] in ${path.relative(projectDir, file)}`,
                    }
                }
            }
        }
