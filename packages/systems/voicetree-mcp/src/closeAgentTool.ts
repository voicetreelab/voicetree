...
export async function closeAgentTool({terminalId, callerTerminalId, forceWithReason}: CloseAgentParams): Promise<McpToolResponse> {
    ...
    if (isSelfClose) {
        const graph: import('@vt/graph-model/graph').Graph = await getConfiguredGraph()
    ...
    if (!isSelfClose) {
        ...
        const graph: import('@vt/graph-model/graph').Graph = await getConfiguredGraph()
        const graphMatchedNodes: Array<{nodeId: string; title: string}> = getNewNodesForAgent(graph, agentName, targetRecord.spawnedAt)
