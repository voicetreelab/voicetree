/**
 * Black-box tests for `ensureVtDaemonForProject` (BF-373).
 *
 * Per CLAUDE.md: no internal mocks. Every assertion resolves to an
 * observable boundary —
 *   - on-disk owner record (`<project>/.voicetree/vtd.owner.json`),
 *   - on-disk port file (`<project>/.voicetree/rpc.port`),
 *   - on-disk auth token (`<project>/.voicetree/auth-token`),
...
beforeEach(async () => {
  const project = await mkdtemp(join(tmpdir(), 'vt-daemon-client-bf373-'))
  await mkdir(join(project, '.voicetree'), { recursive: true })
  harness = { project, spawned: [], externalDaemonPids: [] }
})
...
async function readPersistedOwner(project: string): Promise<OwnerRecord> {
  const raw = await readFile(join(project, '.voicetree', OWNER_FILE), 'utf8')
...
async function readAuthTokenOnDisk(project: string): Promise<string> {
  return (
    await readFile(join(project, '.voicetree', AUTH_TOKEN_FILE), 'utf8')
  ).trim()
}
...
describe('ensureVtDaemonForProject — black-box', () => {
  test('cold-start: spawns one VTD, owner record bound to port, auth-token published', async () => {
    ...
    const onDiskToken = await readAuthTokenOnDisk(harness.project)
  ...
  test('reuse: a second ensure call returns the same pid/port without spawning', async () => {
    const first = await ensureVtDaemonForProject(harness.project, 'electron', {
      bin: FAKE_BIN_COMMAND,
      timeoutMs: 10_000,
    })
    trackDaemonPid(first.pid)

    const second = await ensureVtDaemonForProject(harness.project, 'electron', {
      bin: FAKE_BIN_COMMAND,
      timeoutMs: 5_000,
    })
  ...
  test('stale reclaim (dead pid): record is replaced, new ownerNonce differs', async () => {
    ...
    await writeOwnerRecord(harness.project, {
      ...
      commandFingerprint: {
        ...
        args: [FAKE_BIN, '--project', resolve(harness.project)],
      },
    })
  ...
  test('unsafe owner (live unrelated pid + mismatching health nonce): throws UnsafeOwnerError, owner left intact', async () => {
    ...
    const canonicalProject = resolve(harness.project)
  ...
  test('cooldown suppression: a fresh cooldown breadcrumb makes ensure throw without spawning', async () => {
    ...
    const breadcrumb = {
      ...
      canonicalProject: resolve(harness.project),
      writtenAtMs: now,
      untilMs: now + 60_000,
    ...
    await expect(
      readFile(join(harness.project, '.voicetree', OWNER_FILE), 'utf8'),
...
async function startHealthServerWithNonce(
  ...
  // The test runner's `afterEach` already wipes the project dir; close the
