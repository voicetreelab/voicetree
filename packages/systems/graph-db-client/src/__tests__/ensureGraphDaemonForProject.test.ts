...
beforeEach(async () => {
  const project = await mkdtemp(join(tmpdir(), 'vt-graphd-bf344-'))
  await mkdir(join(project, '.voicetree'), { recursive: true })
  harness = { project, spawned: [], servers: [] }
})
...
describe('ensureGraphDaemonForProject — black-box', () => {
  test('100 concurrent callers cold-start exactly one daemon, one owner record, one port', async () => {
    ...
    const calls = Array.from({ length: callCount }, () =>
      ensureGraphDaemonForProject(harness.project, 'test', {
  ...
  test('existing healthy owner is reused — no new child is spawned', async () => {
    ...
    const result = await ensureGraphDaemonForProject(harness.project, 'test', {
      bin: FAKE_BIN_COMMAND,
      timeoutMs: 2_000,
    })
  ...
  test('stale owner record with dead pid → reclaimed and respawned, single port', async () => {
    ...
    await writeOwnerRecord(harness.project, {
      ...
      commandFingerprint: {
        ...
        args: [FAKE_BIN, '--project-root', resolve(harness.project)],
      },
    })
