...
const PROJECTLESS_DAEMON_SCRIPT = `
...
export type ProjectlessDaemonHandle = {
...
export type SpawnProjectlessDaemonOptions = {
...
export async function spawnProjectlessDaemon(
  opts: SpawnProjectlessDaemonOptions,
): Promise<ProjectlessDaemonHandle> {
  const runtimeCommand = resolveDaemonRuntimeCommand()
  const tsxLoader = opts.tsxLoaderPath ?? requireFromHere.resolve('tsx')
  const child = spawn(runtimeCommand, ['--import', tsxLoader, '--eval', PROJECTLESS_DAEMON_SCRIPT], {
    env: {
      ...process.env,
