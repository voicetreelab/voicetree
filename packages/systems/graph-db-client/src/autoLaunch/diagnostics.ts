 ...
 * unsubscribe handle is returned, emissions are best-effort and non-blocking.
 * No default sink — the protocol must stay quiet unless something explicitly
 * subscribes (operator log, tracing exporter, test capture).
 *
 * Listener failures are swallowed so one broken consumer cannot break the
 ...
 * unsubscribe handle that removes the listener; idempotent across repeat
 * calls.
 */
