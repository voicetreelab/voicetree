// Black-box tests for the graphd SSE frame parser, relocated here as the
// canonical owner of graphd's wire format (RE-PLAN B). graphd sends
// `event: projectedGraph\ndata: <json>\n\n`, carrying the type in the event
// NAME, not a field in the data. An earlier browser parser read only `data:`
// lines and matched on a non-existent `data.type`, so graph updates never
// reached the UI — these tests guard that framing contract.

import {describe, expect, it} from 'vitest'
import {createSseEventParser} from './projectedGraphStream.ts'
