...
describe('resolveTypeForTarget', () => {
    ...
    beforeEach(async () => {
        projectRoot = await realpath(await mkdtemp(join(tmpdir(), 'vt-folder-note-type-')))
    })
    ...
    it('returns the type from the nearest folder note', async () => {
        ...
        const result = resolveTypeForTarget(targetPath, projectRoot)
    ...
    it('prefers the closest ancestor folder note when both declare a type', async () => {
        ...
        const result = resolveTypeForTarget(join(researchDir, 'topic.md'), projectRoot)
    ...
    it('returns undefined when no upstream folder note declares a type', async () => {
        ...
        expect(resolveTypeForTarget(join(workDir, 'topic.md'), projectRoot)).toBeUndefined()
    })
    ...
    it('reads a separate-line scalar after a bare "## Type" heading', async () => {
        ...
        const result = resolveTypeForTarget(join(workDir, 'topic.md'), projectRoot)
    ...
    it('skips a folder note that matches the target itself', async () => {
        ...
        const result = resolveTypeForTarget(join(workDir, 'work.md'), projectRoot)
    ...
    it('returns undefined when the target is outside the project', async () => {
        const outsideDir: string = await realpath(await mkdtemp(join(tmpdir(), 'vt-outside-')))
        try {
            await writeFile(join(outsideDir, 'note.md'), '# Note\n', 'utf8')
            expect(resolveTypeForTarget(join(outsideDir, 'note.md'), projectRoot)).toBeUndefined()
        } finally {
