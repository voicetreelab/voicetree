...
export const CLI_DAEMON_ROUTE_COVERAGE: readonly CliDaemonRouteCoverageEntry[] = [
    ...
    {command: 'vt view set-folder', routeId: 'session.folder-state.set'},
    {
        command: 'GraphDbClient batch folder-state PATCH',
        routeId: 'session.folder-state.batch',
        notes: 'Bulk active-view folder visibility route consumed by the GraphDbClient/renderer; the `vt view set-folder` CLI verb sets one path per call and has no `--batch` flag.',
    },
    {
        command: 'GraphDbClient folder-state GET',
        routeId: 'session.folder-state.read',
        notes: 'Reads active-view folder visibility for the GraphDbClient/renderer; there is no `vt view state` CLI verb (folder state surfaces via `vt view show`).',
    },
    {command: 'vt view list', routeId: 'project.views.list'},
    {
        command: 'GraphDbClient views.create',
        routeId: 'project.views.create',
        notes: 'Named project-view creation route consumed by the GraphDbClient/renderer; there is no `vt view create` CLI verb. CLI view authoring goes through `vt view clone`.',
    },
    {command: 'vt view switch', routeId: 'project.views.activate'},
    {command: 'vt view clone', routeId: 'project.views.clone'},
    {command: 'vt view delete', routeId: 'project.views.delete'},
    {
