...
// fetch issuing real POSTs, real tmpdir vault, real
// `<vault>/.voicetree/agent_metrics.json` and `<vault>/.voicetree/otlp.port`.
...
async function startHarness(): Promise<Harness> {
    ...
    return {vault, port, url: `http://localhost:${port}/v1/metrics`}
}
...
describe('otlpReceiver', (): void => {
    ...
    afterEach(async (): Promise<void> => {
        ...
        if (h !== null) {
            await rm(h.vault, {recursive: true, force: true}).catch((): void => {})
        }
        h = null
    })

    it('binds in the 4318–4327 window and publishes <vault>/.voicetree/otlp.port', async (): Promise<void> => {
        ...
        const published: number | null = await readOtlpPortFile(harness.vault)
        expect(published).toBe(harness.port)
    })

    it('POST /v1/metrics with a fixture payload appends to <vault>/.voicetree/agent_metrics.json', async (): Promise<void> => {
        const harness: Harness = h!
        ...
        const sessions: readonly SessionMetric[] = await getSessions(harness.vault)
