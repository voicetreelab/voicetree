/**
 * Pure command-string transformation that disables Codex's native AGENTS.md
 * project-doc injection for VoiceTree-spawned sessions.
 *
 * VoiceTree already supplies the task/context prompt for spawned Codex
 * sessions, so Codex's own AGENTS.md injection would surface the same repo
 * instructions twice in the model context. We append `-c project_doc_max_bytes=0`
 * right after the `codex` token to suppress it.
 *
 * No I/O — this module just produces strings.
 */
