...
function makeRow(overrides: Partial<CodexThreadRow> = {}): CodexThreadRow {
    return {
        ...
        first_user_message: `prompt\nVOICETREE_TERMINAL_ID = ${TERMINAL} VOICETREE_PROJECT_PATH = ${PROJECT} TASK_NODE_PATH = ${TASK}`,
        cwd: PROJECT,
...
describe('matchCodexThreadId — non-match cases', () => {
    it('returns null on empty input', () => {
        expect(matchCodexThreadId({rows: [], terminalId: TERMINAL, projectRoot: PROJECT, taskNodePath: TASK})).toBeNull()
    })

    it('returns null when the project marker differs (reused terminal name in another project)', () => {
        expect(matchCodexThreadId({
            rows: [makeRow({first_user_message: `VOICETREE_TERMINAL_ID = ${TERMINAL} VOICETREE_PROJECT_PATH = /other PROJECT TASK_NODE_PATH = ${TASK}`})],
            terminalId: TERMINAL, projectRoot: PROJECT, taskNodePath: TASK,
        })).toBeNull()
    })
    ...
    it('returns null when the task path marker differs', () => {
        expect(matchCodexThreadId({
            rows: [makeRow({first_user_message: `VOICETREE_TERMINAL_ID = ${TERMINAL} VOICETREE_PROJECT_PATH = ${PROJECT} TASK_NODE_PATH = /other.md`})],
            terminalId: TERMINAL, projectRoot: PROJECT, taskNodePath: TASK,
        })).toBeNull()
    })
})
...
describe('matchCodexThreadId — defensive parsing', () => {
    it('skips rows with no first_user_message but still returns later valid matches', () => {
        const incomplete: CodexThreadRow = {id: 'no-msg', cwd: PROJECT}
        const result: string | null = matchCodexThreadId({
