...
export const IMPURE_IDENTIFIERS: ReadonlyMap<string, string> = new Map([
    ['fetch', 'network'],
...
export function detectSideEffectsAST(body: ts.Node, aliases: ReadonlyMap<string, string>): { effects: Set<string>; calledNames: Set<string> } {
    ...
    function visit(node: ts.Node): void {
        if (ts.isCallExpression(node)) {
            ...
            if (chain && chain.length >= 2) {
                ...
                if (method === 'emit') effects.add('event-emit')
                const entry = IMPURE_OBJ_METHODS.get(obj)
                if (entry && (entry.methods === '*' || entry.methods.has(method))) effects.add(entry.category)
...
export function extractFunctions(filePath: string, sf: ts.SourceFile): FnEntry[] {
    const fns: FnEntry[] = []
    ...
    function visit(node: ts.Node): void {
        if (ts.isFunctionDeclaration(node) && node.name) push(node.name.text, node.body, node, isExp(node))
        if (ts.isVariableDeclaration(node) && node.initializer && ts.isIdentifier(node.name)) {
            const init = node.initializer
            if (ts.isArrowFunction(init) || ts.isFunctionExpression(init)) push(node.name.text, init.body, node, isExp(node))
        }
        if (ts.isMethodDeclaration(node) && node.name) push(node.name.getText(sf), node.body, node, false)
        ts.forEachChild(node, visit)
    }
...
export function propagateImpurity(allFns: FnEntry[]): void {
        ...
        while (changed) {
            ...
            for (const fn of fileFns) {
                ...
                for (const called of fn.calledNames) {
                    if (impNames.has(called)) { fn.sideEffects = ['transitive']; changed = true; break }
                }
            }
        }
    }
...
export type ArchLayer = 'pure' | 'shell/edge' | 'libraries' | 'systems' | 'UI' | 'other'

export function classifyLayer(f: string): ArchLayer {
...
export type Stats = { totalLoc: number; pureLoc: number; impureLoc: number; fnCount: number; breakdown: Record<string, number> }
...
export function pct(n: number, d: number): string { return d === 0 ? 'N/A' : `${((n / d) * 100).toFixed(1)}%` }
