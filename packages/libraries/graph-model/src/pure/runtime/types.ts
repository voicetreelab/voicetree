...
// `projectRoot`. `writeFolderPath` is for markdown / project content only.
...
export interface GraphModelCallbacks {
  ...
  // Watch folder events
  onProjectSwitching?: () => void | Promise<void>
  onProjectOpened?: (info: WatchingStartedInfo) => void | Promise<void>
  ...
  syncProjectState?: (state: { projectPaths: readonly string[]; writeFolderPath: string | null; starredFolders: readonly string[] }) => void
