...
import type { LayoutEdge, LayoutNode, Pillars, PillarWeights } from './layoutQualityScore';
...
function sq(id: string, x: number, y: number, s = 40, extra: Partial<LayoutNode> = {}): LayoutNode {
...
describe('pillar 1 — node bbox overlap', () => {
  ...
  it('scores by fraction of nodes involved in overlap, not by overlap area', () => {
    // Two coincident boxes: both nodes are compromised → 2/2 → score 0.
    // (The old area-ratio formula scored this 0.5; count-based is stricter.)
    const r = scoreLayout([sq('a', 0, 0), sq('b', 0, 0)], []);
    expect(r.pillars.nodeOverlap).toBe(0);
    expect(r.rawMetrics.overlappingPairCount).toBe(1);
    expect(r.rawMetrics.overlappingNodeCount).toBe(2);
    expect(r.rawMetrics.totalOverlapArea).toBeCloseTo(1600, 6); // retained as diagnostic
  });

  it('is NOT diluted by non-overlapping nodes on a large canvas', () => {
    // 8 well-separated nodes + one coincident overlapping pair = 10 nodes, 2 of
    // which overlap. Count-based → 1 - 2/10 = 0.8. The old area-ratio formula
    // would have hidden this behind the 8 clean nodes' area at ~0.98 — the exact
    // dilution bug this pillar was hardened to remove.
    const clean = Array.from({ length: 8 }, (_, i) => sq(`c${i}`, 1000 * (i + 1), 0));
    const r = scoreLayout([...clean, sq('x', 0, 0), sq('y', 0, 0)], []);
    expect(r.pillars.nodeOverlap).toBeCloseTo(0.8, 6);
    expect(r.rawMetrics.overlappingPairCount).toBe(1);
    expect(r.rawMetrics.overlappingNodeCount).toBe(2);
  });
});
...
describe('pillar 2 — edge crossing', () => {
  it('scores 1.0 for non-crossing edges', () => {
    ...
    const edges: LayoutEdge[] = [{ source: 'a', target: 'b' }, { source: 'c', target: 'd' }];
  ...
  it('counts a proper crossing and normalizes by edge count', () => {
    ...
    const edges: LayoutEdge[] = [{ source: 'a', target: 'c' }, { source: 'b', target: 'd' }];
    const r = scoreLayout(nodes, edges);
