...
//
// Overlap is the single most damaging layout defect — two cards drawn on top of
// each other are unreadable — so this pillar is COUNT-based, scoring the
// fraction of nodes that overlap at least one other node:
//
//     score = 1 - overlappingNodeCount / nodeCount
//
// It deliberately does NOT use an area ratio (overlapArea / totalNodeArea). That
// ratio is diluted toward 1.0 by every non-overlapping node on the canvas, so a
// 271-node graph with dozens of clearly-overlapping cards still scored ~0.99 —
// hiding the one defect users notice first. The fraction-of-nodes formula is
// scale-free and reports exactly "what share of the graph is unreadable": one
// clean layout (0 overlaps) scores 1.0, and overlap can no longer be masked by
// compactness elsewhere. `totalOverlapArea` is retained as a diagnostic only.

export function nodeOverlapPillar(boxes: readonly LayoutBox[]): {
  score: number; totalOverlapArea: number; totalNodeArea: number;
  overlappingPairCount: number; overlappingNodeCount: number;
} {
  let totalNodeArea = 0;
  ...
  for (let i = 0; i < boxes.length; i += 1) {
    for (let j = i + 1; j < boxes.length; j += 1) {
      ...
      if (area > 0) {
        ...
        overlappingNodes.add(j);
      }
    }
  }
  const overlappingNodeCount = overlappingNodes.size;
  const score = boxes.length > 0 ? clamp01(1 - overlappingNodeCount / boxes.length) : 1;
  return { score, totalOverlapArea, totalNodeArea, overlappingPairCount, overlappingNodeCount };
}
