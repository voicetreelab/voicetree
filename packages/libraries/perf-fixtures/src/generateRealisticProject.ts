/**
 * Deterministic project generator for perf harnesses.
 *
 ...
 * is a "realistic" project that the daemon's chokidar+loader path can ingest
 * the same way it would a user's project — far more useful than `mkdtemp`'s
...
/**
 * Compute the in-memory layout of an N-node realistic project.
 *
 ...
 */
export function planProject(nodeCount: number): ProjectLayout {
...
/**
 * Write a realistic project rooted at `projectPath`.
 *
 * Creates `projectPath`, `projectPath/.voicetree/positions.json`,
 * `projectPath/ctx-nodes/`, and one .md file per node. Caller owns `projectPath`
 * lifecycle and cleanup.
 */
export function generateProjectOnDisk(projectPath: string, nodeCount: number): ProjectLayout {
    mkdirSync(projectPath, { recursive: true })
    const voicetreeDir = getProjectDotVoicetreePath(projectPath)
    mkdirSync(voicetreeDir, { recursive: true })
    writeFileSync(join(voicetreeDir, 'positions.json'), '{}', 'utf8')
    mkdirSync(join(projectPath, 'ctx-nodes'), { recursive: true })

    const plan = planProject(nodeCount)
    const createdDirs = new Set<string>()
    for (const node of plan.nodes) {
        const fullPath = join(projectPath, node.relativePath)
