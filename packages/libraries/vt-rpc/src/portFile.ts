// Read and atomic-write of `<vault>/.voicetree/rpc.port`. Plain decimal
...
export async function readRpcPortFile(vaultPath: string): Promise<number | null> {
    try {
        const text: string = await readFile(rpcPortFilePath(vaultPath), 'utf8')
