...
from models import BaseTreeAction, CreateAction, OptimizationResponse, UpdateAction
...
class SingleAbstractionOptimizerAgent(Agent):
    ...
    def _convert_to_typed_actions(self, result: dict[str, Any], node_id: int) -> list[BaseTreeAction]:
        ...
        optimization_data = result.get("single_abstraction_optimizer_response")
        if not isinstance(optimization_data, OptimizationResponse):
            raise RuntimeError(
                f"Expected OptimizationResponse but got {type(optimization_data)}. "
                f"This indicates a problem with the LLM response parsing."
            )
        optimization: OptimizationResponse = optimization_data
        ...
        typed_actions: list[BaseTreeAction] = []

        # Handle original node update if requested
        if optimization.original_new_content:
            # Ensure new_summary is not None (UpdateAction requires it)
            new_summary = optimization.original_new_summary or ""
