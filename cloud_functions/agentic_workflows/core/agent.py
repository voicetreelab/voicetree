...
class Agent:
    ...
    def __init__(self, name: str, state_schema: Union[type[BaseModel], type]):
        """
        Initialize an agent
        ...
        self.post_processors: dict[str, Optional[Callable[[dict[str, Any]], dict[str, Any]]]] = {}  # name -> optional post-processor
        self.model_names: dict[str, Optional[str]] = {}  # name -> optional model name
        self.dataflows: list[tuple[str, str, Optional[Callable[[dict[str, Any]], dict[str, Any]]]]] = []  # (from, to, transform)
        self.conditional_dataflows: list[tuple[str, Callable[[dict[str, Any]], Union[str, list[str]]], Optional[dict[str, Any]]]] = []  # (from, routing_func, path_map)
    ...
    def add_prompt_node(self, name: str, output_schema: type[BaseModel], post_processor: Optional[Callable[[dict[str, Any]], dict[str, Any]]] = None, model_name: Optional[str] = None) -> None:
        """
    ...
    def add_conditional_dataflow(self, from_prompt: str, routing_function: Callable[[dict[str, Any]], Union[str, list[str]]],
                                path_map: Optional[dict[str, Any]] = None) -> None:
        """
    ...
    def set_entry_point(self, prompt_name: str) -> None:
        """Override the default entry point"""
        if prompt_name not in self.prompts:
            raise ValueError(f"Unknown prompt: {prompt_name}")
        self.entry_point = prompt_name

    def compile(self, llm_client: Any = None) -> Any:
        """
        ...
        # Create the graph
        graph = StateGraph(cast(type, self.state_schema))
        ...
        for prompt_name in self.prompts:
            ...
            def make_node_fn(pname: str) -> Callable[[dict[str, Any]], dict[str, Any]]:  # Closure to capture prompt_name
            ...
            # Add node - retry policy removed for langgraph 0.2.57 compatibility
        ...
        for from_prompt, to_prompt, transform in self.dataflows:
            if transform:
                ...
                def make_transformer(t: Callable[[dict[str, Any]], dict[str, Any]], from_p: str, to_p: str) -> Callable[[dict[str, Any]], dict[str, Any]]:
